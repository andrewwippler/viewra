...
const getMoviesWithoutVariantGroup = `-- name: GetMoviesWithoutVariantGroup :many
SELECT
    m.media_id, m.year, m.release_date, m.genre, m.director, m."cast", m.content_rating, m.maturity_rating, m.content_advisories, m.plot, m.tagline, m.original_title, m.sort_title, m.imdb_id, m.tmdb_id, m.runtime_minutes, m.budget, m.revenue, m.original_language, m.country_of_origin, m.awards_summary, m.rating, m.rating_votes,
    med.id as media_id,
    med.library_id,
...
type GetMoviesWithoutVariantGroupRow struct {
	MediaID           int64           `json:"media_id"`
	Year              sql.NullInt64   `json:"year"`
	ReleaseDate       sql.NullTime    `json:"release_date"`
	Genre             sql.NullString  `json:"genre"`
	...
	VariantGroupID    sql.NullInt64   `json:"variant_group_id"`
}

// Gets all movies in a library that don't have a variant_group_id set.
// Used for post-scan reconciliation of variant groups.
func (q *Queries) GetMoviesWithoutVariantGroup(ctx context.Context, libraryID int64) ([]GetMoviesWithoutVariantGroupRow, error) {
	rows, err := q.db.QueryContext(ctx, getMoviesWithoutVariantGroup, libraryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetMoviesWithoutVariantGroupRow{}
	for rows.Next() {
		var i GetMoviesWithoutVariantGroupRow
		if err := rows.Scan(
			&i.MediaID,
			&i.Year,
			&i.ReleaseDate,
...
const listRandomMovies = `-- name: ListRandomMovies :many
SELECT
    m.media_id, m.year, m.release_date, m.genre, m.director, m."cast", m.content_rating, m.maturity_rating, m.content_advisories, m.plot, m.tagline, m.original_title, m.sort_title, m.imdb_id, m.tmdb_id, m.runtime_minutes, m.budget, m.revenue, m.original_language, m.country_of_origin, m.awards_summary, m.rating, m.rating_votes,
    med.id as media_id,
    med.library_id,
...
type ListRandomMoviesRow struct {
	MediaID           int64           `json:"media_id"`
	Year              sql.NullInt64   `json:"year"`
	ReleaseDate       sql.NullTime    `json:"release_date"`
	Genre             sql.NullString  `json:"genre"`
...
func (q *Queries) ListRandomMovies(ctx context.Context, dollar_1 int64) ([]ListRandomMoviesRow, error) {
	rows, err := q.db.QueryContext(ctx, listRandomMovies, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListRandomMoviesRow{}
	for rows.Next() {
		var i ListRandomMoviesRow
		if err := rows.Scan(
			&i.MediaID,
			&i.Year,
			&i.ReleaseDate,
