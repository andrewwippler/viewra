...
const getVariantsByGroupID = `-- name: GetVariantsByGroupID :many
SELECT id, library_id, title, file_path, file_size, file_hash, container_format, duration, width, height, aspect_ratio, codec, codec_profile, bit_rate, frame_rate, scan_type, hdr_format, color_space, color_primaries, thumbnail_path, type, source_type, resolution_label, quality_score, is_3d, stereo_mode, has_dash, dash_manifest_path, transcoding_status, date_added, date_modified, created_at, updated_at, is_extra, audio_codec, language, variant_group_id FROM media
WHERE variant_group_id = $1
ORDER BY language
`

func (q *Queries) GetVariantsByGroupID(ctx context.Context, variantGroupID sql.NullInt64) ([]Medium, error) {
	rows, err := q.db.QueryContext(ctx, getVariantsByGroupID, variantGroupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
