...
interface WebosLivetvPlaySearch {
  libraryId?: number
  channelId?: number
}

const WebosLivetvPlay = () => {
  ...
  // WebOS input navigation
  useWebOSInputNavigation({
    enabled: true,
    rowSelector: '[data-tv-button]',
    onBackPress: () => navigate({ to: '/webos/livetv', search: { libraryId } }),
  })
  ...
  const handleClose = useCallback(() => {
    navigate({ to: '/webos/livetv', search: { libraryId } })
  }, [navigate, libraryId])
...
export const Route = createFileRoute('/webos/livetv/play')({
  validateSearch: (search: Record<string, unknown>): WebosLivetvPlaySearch => ({
    libraryId: Number(search.libraryId) || undefined,
    channelId: Number(search.channelId) || undefined,
  }),
  component: WebosLivetvPlay,
})