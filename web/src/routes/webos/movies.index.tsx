...
import { getApiMovies } from '@/lib/api/generated/movies/movies'
import type { GithubComMantonxViewraInternalApplicationMoviesMovieResponse } from '@/lib/api/generated/models'
...
const WebosMovies = () => {
  ...
  const movies = res && res.status === 200 ? res.data.movies || [] : []
      ...
      <div className="px-1 pb-6">
        {isLoading ? (
          ...
          <div className="text-center text-neutral-500 py-20">
            <Film className="w-8 h-8 mx-auto mb-4 opacity-50" />
            <p className="text-xl mb-2">No movies found</p>
            <p className="text-base">Try a different search or add movies to your library</p>
          </div>
        ) : (
          chunk(movies, 5).map((group, rowIndex) => (
            <div data-row key={rowIndex} className="grid grid-cols-5 gap-2">
              {group.map((movie: GithubComMantonxViewraInternalApplicationMoviesMovieResponse) => (
                <button
                  key={movie.id}
                  type="button"
                  id={rowIndex === 0 && group.indexOf(movie) === 0 ? 'connect' : undefined}
                  className="border-none bg-transparent text-left cursor-pointer p-0.5"
                  onClick={() => navigate({ to: `/webos/movies/${movie.id}` })}
                >
                  ...
                  <div className="mt-1 px-0.5 min-h-[2rem]">
                    <h3 className="text-white text-sm font-medium w-full truncate" dangerouslySetInnerHTML={{ __html: breakTitle(movie.title || 'Untitled') }} />
                  </div>
                </button>
              ))}
            </div>
...
export const Route = createFileRoute('/webos/movies/')({
  component: WebosMovies,
})
