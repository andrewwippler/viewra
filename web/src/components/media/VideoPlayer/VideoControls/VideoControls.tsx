...
export const VideoControls = ({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Linear remote focus traversal (Left / Right Arrows)
      if (
        (e.key === 'ArrowLeft' || e.key === 'ArrowRight') &&
        container.contains(document.activeElement)
      ) {
        const controls = getFocusableControls()
        // If the timeline element currently holds active focus, let it intercept arrow keys internally
        if (document.activeElement === timelineRef.current) {
          return
        }
        ...
        if (controls.length > 0) {
          ...
          if (e.key === 'ArrowRight') {
            nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % controls.length
          } else {
            nextIndex = currentIndex === -1 ? controls.length - 1 : (currentIndex - 1 + controls.length) % controls.length
          }
  ...
  return (
    <>
      {metadata && (
        <div
          className={`absolute top-0 left-0 right-0 transition-opacity duration-300 z-10 ${
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 transition-opacity duration-300 z-10 ${
          showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
      >
        ...
        <div ref={controlsRef} className="relative px-6 pb-6 pt-16">
          ...
          <div className="flex items-center justify-between text-white">
            <div className="flex items-center gap-2">
              ...
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M4 13c0 4.42 3.58 8 8 8s8-3.58 8-8h-2c0 3.31-2.69 6-6 6s-6-2.69-6-6 2.69-6 6-6v4l5-5-5-5v4c-4.42 0-8 3.58-8 8zm7.9 3h-.85v-3.26l-1.01.31v-.69l1.77-.63h.09V16zm4.28-1.76c0 .32-.03.6-.1.82s-.17.42-.29.57-.28.26-.45.33-.37.1-.59.1-.41-.03-.59-.1-.33-.18-.46-.33-.23-.34-.3-.57-.11-.5-.11-.82v-.74c0-.32.03-.6.1-.82s.17-.42.29-.57.28-.26.45-.33.37-.1.59-.1.41.03.59.1.33.18.46.33.23.34.3.57.11.5.11.82v.74zm-.85-.86c0-.19-.01-.35-.04-.48s-.07-.23-.12-.31-.11-.14-.19-.17-.16-.05-.25-.05-.18.02-.25.05-.14.09-.19.17-.09-.19-.11-.32-.04-.29-.04-.48v-.97z" />
                </svg>
              </button>
